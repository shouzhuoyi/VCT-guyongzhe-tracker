  ...
  dateFolders.forEach((dateFolder) => {
    ...
    jsonFiles.forEach((file) => {
      const content = JSON.parse(
        fs.readFileSync(path.join(currentDir, file), "utf8"),
      );
...
const filteredPlayersList = allPlayersList.filter((p) =>
  allowedPlayers.has(p.name),
);

// Sort by Rating default
filteredPlayersList.sort((a, b) => b.rating - a.rating);
...
fs.writeFileSync(
  ...
  statsTemplate({
    ...
    jsonPlayers: JSON.stringify(filteredPlayersList),
  }),
);
